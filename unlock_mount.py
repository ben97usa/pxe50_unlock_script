...
def gp_format_and_mount_data(child):
    ok1, out1 = gp_send_cmd(
        child,
        "sudo mkfs.ext4 -F /dev/mmcblk0p5",
        timeout=300,
        step_desc="Format /dev/mmcblk0p5 as ext4"
    )
    if not ok1:
        return False, out1

    ok2a, out2a = gp_send_cmd(
        child,
        "mkdir -p /vol/data",
        timeout=20,
        step_desc="Create /vol/data"
    )
    if not ok2a:
        return False, out1 + "\n" + out2a
    ...
    ok2, out2 = gp_send_cmd(
        child,
        "mount /dev/mmcblk0p5 /vol/data",
        timeout=60,
        step_desc="Mount /dev/mmcblk0p5 to /vol/data"
    )
    if not ok2:
        return False, out1 + "\n" + out2a + "\n" + out2

    ok3, out3 = gp_send_cmd(
        child,
        "e2label /dev/mmcblk0p5 DATA",
        timeout=30,
        step_desc="Set filesystem label DATA"
    )
    if not ok3:
        return False, out1 + "\n" + out2a + "\n" + out2 + "\n" + out3
...
def gp_get_fpga_bootmode(child):
    ok, output = gp_send_cmd(
        child,
        "cerberus_utility fpgagetbootmode",
        timeout=30,
        step_desc="Get FPGA boot mode"
    )
    ...
    if m:
        return m.group(1), output

    return None, output

def gp_set_fpga_bootmode_1(child):
    ok, output = gp_send_cmd(
        child,
        "cerberus_utility fpgasetbootmode 1",
        timeout=30,
        step_desc="Set FPGA boot mode to 1"
    )
    return ok, output
...
def main():
    ...
    for slot in slots:
        ...
        try:
            ...
            if policy2 == "0x2":
                ...
                # =====================================================
                # AFTER UNLOCK:
                # 1) format /dev/mmcblk0p5
                # 2) mount to /vol/data
                # 3) label DATA
                # 4) get FPGA boot mode
                # 5) if boot mode == 0 -> set to 1
                # =====================================================
                fmt_ok, fmt_output = gp_format_and_mount_data(child)
                if not fmt_ok:
                    print_fail(f"Slot {slot}: post-unlock format/mount failed")
                    if fmt_output and fmt_output.strip():
                        print(fmt_output.strip())
                    slot_status[slot] = f"unlock success but format/mount failed - {gp_sn}"
                    failed_slots.append((slot, f"unlock success but format/mount failed - {gp_sn}"))
                    gp_exit(child)
                    continue

                boot_mode, boot_output = gp_get_fpga_bootmode(child)
                if boot_output and boot_output.strip():
                    print(boot_output.strip())

                if boot_mode is None:
                    print_fail(f"Slot {slot}: cannot read FPGA boot mode")
                    slot_status[slot] = f"unlock success but cannot read boot mode - {gp_sn}"
                    failed_slots.append((slot, f"unlock success but cannot read boot mode - {gp_sn}"))
                    gp_exit(child)
                    continue

                print_info(f"Slot {slot}: current FPGA boot mode = {boot_mode}")

                if boot_mode == "0":
                    print_warn(f"Slot {slot}: FPGA boot mode is 0, need set to 1")
                    set_ok, set_output = gp_set_fpga_bootmode_1(child)
                    if set_output and set_output.strip():
                        print(set_output.strip())

                    if not set_ok:
                        print_fail(f"Slot {slot}: failed to set FPGA boot mode to 1")
                        slot_status[slot] = f"unlock success but set boot mode failed - {gp_sn}"
                        failed_slots.append((slot, f"unlock success but set boot mode failed - {gp_sn}"))
                        gp_exit(child)
                        continue

                    # verify again
                    boot_mode2, boot_output2 = gp_get_fpga_bootmode(child)
                    if boot_output2 and boot_output2.strip():
                        print(boot_output2.strip())

                    if boot_mode2 == "1":
                        print_ok(f"Slot {slot}: post-unlock steps all success")
                        slot_status[slot] = f"unlock + format + bootmode success - {gp_sn}"
                        post_unlock_success.append((slot, gp_sn))
                    else:
                        print_fail(f"Slot {slot}: boot mode still not 1")
                        slot_status[slot] = f"unlock success but boot mode still not 1 - {gp_sn}"
                        failed_slots.append((slot, f"unlock success but boot mode still not 1 - {gp_sn}"))

                else:
                    print_ok(f"Slot {slot}: FPGA boot mode already {boot_mode}")
                    print_ok(f"Slot {slot}: post-unlock steps all success")
                    slot_status[slot] = f"unlock + format + bootmode success - {gp_sn}"
                    post_unlock_success.append((slot, gp_sn))
            else:
    ...
    print("\nPost-unlock full success:")
    if post_unlock_success:
        for slot, gp_sn in post_unlock_success:
            print(f"  - slot {slot}: {gp_sn}")
    else:
        print("  (none)")
